...
export default function VaporizeTextCycle({
  ...
  const globalDpr = useMemo(() => {
    if (typeof window !== "undefined") {
      // Cap DPR — a high ratio multiplies canvas pixels (and particle count),
      // which is the main source of jank. 1.25 stays crisp but far lighter.
...
const createParticles = (
  ...
  const sampleRate = Math.max(2, Math.round(baseSampleRate)); // sample fewer pixels → fewer particles (much smoother)
